...
using StrohisUploadLib.Playlists;
...
namespace StrohisUploadLib.Communication.Youtube
{
	internal static class WebService
	{
		...
		internal static string ObtainAccessToken(string code)
		{
			// Content zusammenbauen
			string content = resourceManager.GetString("AuthCodeContent");

			content = string.Format(content, code, clientId, clientSecret);
			...
			request.Headers.Add(resourceManager.GetString("AuthCodeContentType"));

			return WebService.Communicate(request, bytes);
		}
		...
		internal static void RevokeAccess(Account account)
		{
			...
			if (!string.IsNullOrWhiteSpace(token))
			{
				...
				var status = WebService.Communicate(request);
		...
		{
			var lastbyte = WebService.CheckUploadStatus(ref video, url);
			...
			if (lastbyte == -1)
			{
				request = WebService.CreateNewUploadRequest(ref video, url);
			}
			else
			{
				request = WebService.CreateResumeUploadRequest(ref video, url, lastbyte);
